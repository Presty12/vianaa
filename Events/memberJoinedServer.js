...
module.exports = async (member) => {
  ...
  if(jaildekiler.some(x => x.includes(member.id)) || tempjaildekiler.some(x => x.id === member.id)){
    ...
    if(ayarlar.fakeHesapLogKanali && member.guild.channels.cache.has(ayarlar.fakeHesapLogKanali)) return member.guild.channels.cache.get(ayarlar.teyitKanali).send(new MessageEmbed().setAuthor(member.guild.name, member.guild.iconURL({dynamic: true})).setDescription(`${member} üyesi sunucuya katıldı fakat hesabı ${member.client.tarihHesapla(member.user.createdAt)} açıldığı için jaile atıldı!`).setTimestamp().setFooter("Viana Moderation"));
  ...
  .setDescription(`
  **☆ Viana'ya hoş geldin ${member}, seninle toplam \`${member.guild.memberCount}\` kişiyiz!**
  **☆ Yan taraftaki ses kanallarına girerek kaydını yaptırabilirsin.**
  **☆ Seninle <@&759433911100768277> || <@&760404936592326666> ilgilenecektir.**
  **☆ Hesabının açılış tarihi: ${member.client.tarihHesapla(member.user.createdAt)}**
  **☆ Hesabınız ${guvenilirlik ? "kayıt olmaya uygun değildir!" : "kayıt olmaya uygundur."}**
  `);
