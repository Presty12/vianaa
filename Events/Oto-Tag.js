...
module.exports = (oldUser, newUser) => {
  ...
  if(!user) return;
  const embed = new discord.MessageEmbed().setAuthor(user.displayName, user.user.avatarURL({dynamic: true})).setFooter("Viana Moderation").setColor(client.randomColor()).setTimestamp();
  ...
  if(newUser.username.includes(ayarlar.tag) && !user.roles.cache.has(ayarlar.ekipRolu)){
      ...
      if(ayarlar.ekipLogKanali && log) log.send(embed.setDescription(`${user} kişisi tagımızı (\`${ayarlar.tag}\`) aldığı için <@&${ayarlar.ekipRolu}> rolü verildi`).setColor("RANDOM")).catch();
      ...
      if(ayarlar.ekipLogKanali && log) log.send(embed.setDescription(`${user} kişisi tagımızı (\`${ayarlar.tag}\`) çıkardığı için <@&${ayarlar.ekipRolu}> rolü alındı`).setColor("RANDOM")).catch();
  }
}
