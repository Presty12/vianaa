...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  ...
  if (message.member.hasPermission("ADMINISTRATOR")) {
    ...
    message.channel.send(`${uye}`, {embed: embed.setAuthor(uye.displayName, uye.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${message.author} seni yanına çekmek istiyor. Onaylıyor musun?`)}).then(async msj => {
      await msj.react('✅');
