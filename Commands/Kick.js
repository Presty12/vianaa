...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!ayar.banciRolleri) return message.channel.send(embed.setDescription("Sunucuda herhangi bir `BAN` rolü tanımlanmamış.")).then(x => x.delete({timeout: 5000}));
  ...
  if(ayar.banLogKanali && client.channels.cache.has(ayar.banLogKanali)) client.channels.cache.get(ayar.banLogKanali).send(new MessageEmbed().setColor(client.randomColor()).setTimestamp().setFooter("Viana Moderation").setTitle('Üye Kicklendi!').setDescription(`**Kickleyen Yetkili:** ${message.author} (${message.author.id})\n**Kicklenen Üye:** ${victim.user.tag} (${victim.user.id})\n**Sebep:** ${reason}`));
};
